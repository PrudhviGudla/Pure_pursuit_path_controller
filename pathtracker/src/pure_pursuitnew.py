...
class purepursuit():
    ...
    def __init__(self):
        rospy.init_node('pure_pursuitnew', anonymous=True)
        ...
        #self.cy.fill(5)
        #self.cy = np.zeros(1000, dtype= float)
    ...
    def pure_pursuit(self):
        final_signal = Twist()
        min_ind ,flag = self.search_target_index()
        if flag == 0:
    ...
    def search_target_index(self):
        ...
        if self.old_nearest_point_index is None:
            ...
            distance_this_index = self.calc_distance(self.cx[ind],
                                                      self.cy[ind])
            while True:
